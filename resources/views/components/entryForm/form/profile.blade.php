...
<section>
    ...
    <table  class="body {{$isOpen ? 'open' : 'close'}}">
        ...
        @if($list && count($list)>0)
            ...
            @case('name')
            <tr class="{{($groupName)}}">
                <th>氏名@if($list['required'])<span class="required">必須</span>@endif</th>
                <td class="">
                @foreach($list As $fieldName => $rules)
                    ...
                    <div class="fields name">
                        ...
                        <x-molecules.input type="text" name="lastName" id="lastName" :value="old('lastName', $fEntUserApplyInfo->nameSei ?? '')" maxlength="16" placeholder="例）天職" />
                    ...
                    <div class="fields name">
                        ...
                        <x-molecules.input type="text" name="firstName" id="firstName" :value="old('firstName', $fEntUserApplyInfo->nameMei ?? '')" maxlength="16" placeholder="例）太郎" />
            ...
            @case('nameRuby')
            <tr class="{{($groupName)}}">
                <th>氏名カナ@if($list['required'])<span class="required">必須</span>@endif</th>
                <td class="">
                @foreach($list As $fieldName => $rules)
                    ...
                    <div class="fields name">
                        ...
                        <x-molecules.input type="text" name="lastKana" id="lastKana" :value="old('lastKana', $fEntUserApplyInfo->nameSeiKn ?? '')" maxlength="32" placeholder="例）テンショク" />
                    ...
                    <div class="fields name">
                        ...
                        <x-molecules.input type="text" name="firstKana" id="firstKana" :value="old('firstKana', $fEntUserApplyInfo->nameMeiKn ?? '')" maxlength="32" placeholder="例）タロウ" />
            ...
            @case('birthday')
            <tr class="{{($groupName)}}">
                <th>生年月日@if($list['required'])<span class="required">必須</span>@endif</th>
                <td class="multi">
                @foreach($list As $fieldName => $rules)
                    @switch($fieldName)
            ...
            @case('address')
            <tr class="{{($groupName)}}">
                <th>住所@if($list['required'])<span class="required">必須</span>@endif</th>
                <td>
                    <dl>
                    @foreach($list As $fieldName => $rules)
...
{{--                            @if($rules['required'])<i class="required pcLayout">*</i>@endif--}}
            ...
            @case('telNumber')
            <tr class="{{($groupName)}}">
                <th>電話番号@if($list['required'])<span class="required">必須</span>@endif</th>
                <td>
                @foreach($list As $fieldName => $rules)
            ...
            @case('mailAddress')
            <tr class="{{($groupName)}}">
                <th>メールアドレス@if($list['required'])<span class="required">必須</span>@endif</th>
                <td>
                @foreach($list As $fieldName => $rules)
                    ...
                    @case('mailAddress')
                    <div class="fields">
                        <x-molecules.input type="email" name="mailAddress" id="mailAddress" class="email01" :value="old('mailAddress', $fEntUserApplyInfo->mailAddress ?? '')" placeholder="例）abc@xx.ne.jp" />
                ...
                @endforeach
                    ...
                    <div class="notice">
                        ※携帯電話可。迷惑メール対策等で、ドメイン指定受信されている場合は、弊社からのメールが正しく届かない場合はございます。<br>メールドメイン「@1049.cc」を受信できるよう設定してください。
                    </div>
            ...
            <tr class="{{($groupName)}}">
                <th>現在の職業@if($list['required'])<span class="required">必須</span>@endif</th>
                <td>
                @foreach($list As $fieldName => $rules)
            ...
            <x-entryForm.form.custom :fEntUserApplyInfo="$fEntUserApplyInfo" :groupName="$groupName" :list="$list" />
