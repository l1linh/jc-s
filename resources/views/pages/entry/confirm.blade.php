                        ...
                        <section class="mod_apply_confirm">
                            <form id="{{$path}}ConfirmForm" name="fm" method="post" action="{{$page->action ?? '#'}}">
                                ...
                                @foreach($params as $key => $value)
                                    ...
                                    @else
                                        @foreach($value as $index => $eachValue)
                                        <input type="hidden" name="{{$key}}[]" value="{{$eachValue}}" />
                                        @endforeach
                                    @endif
                                @endforeach
                                ...
                                if($arrayCustom && count($arrayCustom)>0) {
                                    foreach($arrayCustom As $name => $rules) {
                                        if(isset($params[$name])) {
                                            if($rules['fieldType'] === 'input' or $rules['fieldType'] === 'textarea') {
                                                $customConfirmList[$rules['section']][$name]['value'] = $params[$name];
                                                $customConfirmList[$rules['section']][$name]['hidden'] = $rules['hidden'];
                                            }
                                            if($rules['fieldType'] === 'select' || $rules['fieldType'] === 'radio') {
                                                $customConfirmList[$rules['section']][$name]['value'] = $rules['master'][$params[$name]]??null;
                                            }
                                            if($rules['fieldType'] === 'checkbox') {
                                                $checkedValueList = [];
                                                foreach ($params[$name] as $param) {
                                                    if (isset($rules['master'][$param])){
                                                        $checkedValueList[]= $rules['master'][$param];
                                                    }
                                                }
                                                $customConfirmList[$rules['section']][$name]['value'] =  implode(" , ", $checkedValueList);
                                            }
                                        }
                                    }
                                }
