...
class L2CreateApplicantData extends L2Abstract
{
    ...
    protected function exec(MsgL2CreateApplicantData $msg)
    {
        ...
        //カスタムselect、checkbox、radio選択項目の値=>名称への変換用配列
            ...
            foreach($fEntConfig->backendSettings['form'][$formPath]['custom'] As $sectionName => $items) {
                foreach($items As $groupName => $list) {
                    ...
                    foreach($list As $fieldName => $rules) {
                        ...
                        if(isset($rules['fieldType']) && (($rules['fieldType'] === 'select')  || ($rules['fieldType'] === 'checkbox') || ($rules['fieldType'] === 'radio')) ) {
        ...
        foreach($params As $name => $value) {
            ...
            if(isset($customSelectItems[$name]) && ($value !== null)) {
                ...
                // 選択値を配列で扱う。(checkboxの場合、配列で受け取るため)
                $selectedValList = array();
                if (!is_array($value)) {
                    $selectedValList[] = $value;
                } else {
                    $selectedValList = $value;
                }

                // 選択肢の値に紐づく名称を格納。
                $selectedNmList = array();
                foreach ($selectedValList as $selectedVal) {
                    if (isset($customSelectItems[$name][(int)$selectedVal])) {
                        $selectedNmList[]= $customSelectItems[$name][(int)$selectedVal];
                    }
                }
                $fEntApplyFreeItem->value = implode(" , ", $selectedNmList);
            } else {
